...
const app = {
    ...
    importBook(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                importedBook.id = 'book_' + Date.now(); // Generate new ID
                
                // Limit check for free users
                if (auth.userData && auth.userData.subscriptionStatus !== 'pro') {
                    if (this.state.books.length >= 1) {
                        app.showToast("Free Plan Limit: Max 1 Book allowed. Upgrade to import!", "fa-crown");
                        app.navigateTo('subscription');
                        return;
                    }
                    if (importedBook.pages && importedBook.pages.length > 3) {
                        app.showToast("This imported book has more than 3 pages. Upgrade to PRO to import it!", "fa-crown");
                        app.navigateTo('subscription');
                        return;
                    }
                }
    ...
    // --- NEW: BACKUP PROJECT ENGINE (.mfb) ---
    exportProject() {
        if (auth.userData && auth.userData.subscriptionStatus !== 'pro') {
            app.showToast("Exporting project is a PRO feature. Upgrade to unlock!", "fa-crown");
    ...
    // --- NEW: 100% ACCURATE PDF EXPORT ENGINE ---
    exportAsPDF() {
        this.closeModal('share-modal');
        this.showToast("Preparing High-Quality PDF...", "fa-cog fa-spin");
        ...
        setTimeout(() => {
            alert("Instructions:\n1. Keep 'Background graphics' ENABLED in print settings.\n2. Save as PDF.");
            window.print();
            printContainer.innerHTML = ''; // Clear memory
        }, 1000);
    }
};
